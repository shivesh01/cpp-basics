    ...
    else
    {
        ...
        while (cursor->IS_LEAF == false)
        {
            for (int i = 0;
                 i < cursor->size; i++)
            {
                // If the element to be found is not present
                if (x < cursor->key[i])
                {
                    cursor = cursor->ptr[i];
                    break;
...
// Functio to implement the Insert
// Opertation in B+ Tree
