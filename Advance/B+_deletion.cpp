...
node *insert)into_node(node *root, node *n, int left_index, int key, node *right)
{
    int i;
    ...
    node *insert_into_node_after_splitting(node * root, node * old_node, int left_index, int key, node *right)
    {
        ...
        for (i = 0, j = 0; i < old_node->num_keys;
             i++, j++)
        {
            if (j == left_index)
                j++;
    ...
    node *coalesce_nodes(node * root, node * n, node * neighbor, int neighbor_index, int k_prime)
    {
        ...
        neighbor_insertion_index = neighbot->num_keys;
